...
func Validate(token string) *ValidateUserResponse {
	...
	resp, _ := Unmarshal[ValidateUserResponse](converter)
	return &resp
}
...
func (u *User) Validate(c *gin.Context) bool {
	...
	if password, err := cache.Get(c, fmt.Sprintf("fystart:user:%s", u.Username)).Result(); err == nil && len(password) > 0 {
	...
	cache.Set(c, fmt.Sprintf("fystart:user:%s", u.Username), u.Password, 30*time.Minute)
	return true
}
